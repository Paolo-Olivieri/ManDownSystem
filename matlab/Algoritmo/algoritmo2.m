% Correzione del problema riscontrato in "algoritmo.m". Infatti nel momento
% in cui si trova una valle, si verifica l'esistenza di un picco (poi viene
% selezionato il massimo) entro 1s dalla valle trovata. Dopo aver trovato
% il picco massimo, si identificano le valli comprese tra la prima valle
% trovata e il picco massimo e si seleziona la minima. 
% In questo modo, è poi possibile andare ad identificare il TransitionTime (TT) e, quindi, le
% regioni per il calcolo della variazione della pressione.

% Un problema persistente è che le scale vengono considerate come cadute
...
th_peak = 2;                % [g]   Threshold per il picco
th_pressure_fall = 0.09;    % [hPa] Threshold per variazione di pressione [hPa]. considerando 0.12hPa/m * 0.8m
...
while i<=length(acc)
    ...
    if acc(i) < th_valley
        ...
        window_end =min(i + round(t_peakWindow / mean(diff(time))), length(acc));
        % Ricerca del picco
        if any(acc(i:window_end) > th_peak)

            % Preleva picco massimo
            [peak_value, peak_relative_index] = max(acc(i:window_end));
            peak_index = peak_relative_index + i - 1;

            % Trova tutti gli indici dove acc < th_valley
            valley_indices = find(acc(i:peak_index) < th_valley) + i - 1;
            ...
            if ~isempty(valley_indices)
                ...
                postTransition_start_index = round(TT_index + 1/mean(diff(time)));
...
yline(th_movement,'LineStyle','--','Color','k','LineWidth',1.5, ...
    'Label','Movement Threshold','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','FontSize',14);
...
if fall_alert
    ...
    text((postTransition_end + time(recovery_end)) / 2, peak_value - 0.6, 'Recovery Area', ...
        'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 15, 'FontWeight', 'bold');
